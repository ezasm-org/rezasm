...
impl Memory {
    ...
    pub fn read_bytes(&self, address: usize, count: usize) -> Result<RawData, EzasmError> {
        if address < self.disallowed_bytes || address + count > self.memory_size {
            return Err(EzasmError::ReadOutOfBoundsError(address));
        }
        Ok(RawData::new(&self.memory[address..address + count]))
    }

    pub fn read(&self, address: usize) -> Result<RawData, EzasmError> {
        self.read_bytes(address, self.word_size.value())
    }

    pub fn write(&mut self, address: usize, data: &RawData) -> Result<(), EzasmError> {
        if address + data.data.len() > self.memory_size {
            Err(EzasmError::WriteOutOfBoundsError(address))
        } else {
            self.unsafe_write(address, data)
        }
    }

    pub fn unsafe_write(&mut self, address: usize, data: &RawData) -> Result<(), EzasmError> {
        if address + data.data.len() > self.memory_size {
            Err(EzasmError::WriteOutOfBoundsError(address))
        } else {
    ...
    pub fn add_string_immediates(&mut self, strings: Vec<&String>) -> Result<(), EzasmError> {
